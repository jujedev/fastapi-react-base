...
import { getActivePower } from "services/energyApi";
...
export default function ActivePowerChart({ range }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const result = await getActivePower(range);
        setData(result);
      } catch (err) {
      ...
      >
        <defs>
          ...
          <linearGradient id="gradientL2" gradientTransform="rotate(90)">
            <stop offset="10%" stopColor={alpha(theme.palette.info.main, 0.4)} />
            <stop offset="90%" stopColor={alpha(theme.palette.background.default, 0.4)} />
          </linearGradient>
          ...
          <linearGradient id="gradientTotal" gradientTransform="rotate(90)">
            <stop offset="10%" stopColor={alpha(theme.palette.warning.main, 0.4)} />
            <stop offset="90%" stopColor={alpha(theme.palette.background.default, 0.4)} />
          </linearGradient>
        </defs>
      </LineChart>
...
ActivePowerChart.propTypes = { range: PropTypes.string };
